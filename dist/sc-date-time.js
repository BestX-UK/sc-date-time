      ...
      link: function(scope, element, attrs, ngModel) {
        ...
        attrs.$observe('mindate', function(val) {
          if ((val != null) && Date.parse(val)) {
            ...
            return scope.restrictions.mindate.setUTCHours(0, 0, 0, 0);
          }
        });
        attrs.$observe('maxdate', function(val) {
          if ((val != null) && Date.parse(val)) {
            scope.restrictions.maxdate = new Date(val);
            return scope.restrictions.maxdate.setUTCHours(23, 59, 59, 999);
          }
        });
      ...
      controller: [
        '$scope', 'scDateTimeI18n', function(scope, scDateTimeI18n) {
          ...
          scope.setDate = function(newVal, save) {
            ...
            scope.clock._hours = scope._hours24 ? scope.date.getUTCHours() : scope.date.getUTCHours() % 12;
          ...
          scope.display = {
            fullTitle: function() {
              var _timeString;
              _timeString = scope._hours24 ? 'HH:mm:ss.sss' : 'h:mm:ss.sss a';
              if (scope._displayMode === 'full' && !scope._verticalMode) {
                return _dateFilter(scope.date, "EEEE d MMMM yyyy, " + _timeString, "UTC");
              } else if (scope._displayMode === 'time') {
                return _dateFilter(scope.date, _timeString, "UTC");
              } else if (scope._displayMode === 'date') {
                return _dateFilter(scope.date, 'EEE d MMM yyyy', "UTC");
              } else {
                return _dateFilter(scope.date, "d MMM yyyy, " + _timeString, "UTC");
              }
            },
            title: function() {
              if (scope._mode === 'date') {
                return _dateFilter(scope.date, (scope._displayMode === 'date' ? 'EEEE' : "EEEE " + (scope._hours24 ? 'HH:mm' : 'h:mm a')), "UTC");
              } else {
                return _dateFilter(scope.date, 'MMMM d yyyy', "UTC");
              }
            },
            "super": function() {
              if (scope._mode === 'date') {
                return _dateFilter(scope.date, 'MMM', "UTC");
              } else {
                return '';
              }
            },
            main: function() {
              return $sce.trustAsHtml(scope._mode === 'date' ? _dateFilter(scope.date, 'd', "UTC") : scope._hours24 ? _dateFilter(scope.date, 'HH:mm', "UTC") : (_dateFilter(scope.date, 'h:mm', 'UTC')) + "<small>" + (_dateFilter(scope.date, 'a', 'UTC')) + "</small>");
            },
            sub: function() {
              if (scope._mode === 'date') {
                return _dateFilter(scope.date, 'yyyy', 'UTC');
              } else {
                return _dateFilter(scope.date, 'HH:mm', 'UTC');
              }
            }
          };
          scope.calendar = {
            ...
            _allMonths: (function() {
              ...
              results = [];
              for (i = j = 0; j <= 11; i = ++j) {
                results.push(_dateFilter(new Date(Date.UTC(0, i)), 'MMMM'));
              }
              return results;
            })(),
            offsetMargin: function() {
              return (new Date(Date.UTC(this._year, this._month)).getUTCDay() * 2.7) + "rem";
            },
            isVisible: function(d) {
              return new Date(Date.UTC(this._year, this._month, d)).getUTCMonth() === this._month;
            },
            isDisabled: function(d) {
              var currentDate, maxdate, mindate;
              currentDate = new Date(Date.UTC(this._year, this._month, d));
            ...
            isPrevMonthButtonHidden: function() {
              ...
              return (date != null) && this._month <= date.getUTCMonth() && this._year <= date.getUTCFullYear();
            },
            ...
            "class": function(d) {
              var classString;
              classString = '';
              if ((scope.date != null) && new Date(Date.UTC(this._year, this._month, d)).getTime() === new Date(scope.date.getTime()).setUTCHours(0, 0, 0, 0)) {
                classString += "selected";
              }
              if (new Date(Date.UTC(this._year, this._month, d)).getTime() === new Date().setUTCHours(0, 0, 0, 0)) {
                classString += " today";
              }
              return classString;
            },
            select: function(d) {
              scope.date.setUTCFullYear(this._year, this._month, d);
            ...
            monthChange: function(save) {
              ...
              mindate = scope.restrictions.mindate;
              maxdate = scope.restrictions.maxdate;
              if ((mindate != null) && mindate.getUTCFullYear() === this._year && mindate.getUTCMonth() >= this._month) {
                this._month = Math.max(mindate.getUTCMonth(), this._month);
              }
              if ((maxdate != null) && maxdate.getUTCFullYear() === this._year && maxdate.getUTCMonth() <= this._month) {
                this._month = Math.min(maxdate.getUTCMonth(), this._month);
              }
              scope.date.setUTCFullYear(this._year, this._month);
              if (scope.date.getUTCMonth() !== this._month) {
                scope.date.setUTCDate(0);
              }
            ...
            yearChange: function(save) {
              ...
              mindate = scope.restrictions.mindate;
              maxdate = scope.restrictions.maxdate;
              i = (mindate != null) && mindate.getUTCFullYear() === scope.calendar._year ? mindate.getUTCMonth() : 0;
              len = (maxdate != null) && maxdate.getUTCFullYear() === scope.calendar._year ? maxdate.getUTCMonth() : 11;
          ...
          scope.clock = {
            ...
            isAM: function() {
              return scope.date.getUTCHours() < 12;
            }
          };
          scope.$watch('clock._seconds', function(val, oldVal) {
            if ((val != null) && val !== (scope.date.getUTCSeconds() + scope.date.getUTCMilliseconds() * 0.001) && !isNaN(val) && (0 <= val && val <= 59)) {
          ...
          scope.$watch('clock._minutes', function(val, oldVal) {
            if ((val != null) && val !== scope.date.getUTCMinutes() && !isNaN(val) && (0 <= val && val <= 59)) {
